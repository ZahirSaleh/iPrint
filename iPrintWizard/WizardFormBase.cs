...
namespace WizardFormLib
{
	public partial class WizardFormBase : Form
	{
		...
		private void graphicPanelTop_Paint(object sender, PaintEventArgs e)
		{
			...
			// the rectangle to be used for the gradient - by default, it's the same as 
			// the panel
			...
			try
			{
				// retrieve the image if necessary, resize it if necessary, and position it in the panel
				if (this.GraphicPanelImageResource.Length > 0)
				{
					// since this code is in a DLL, and since the bitmap is located in the 
					// exe's resources, we need to get the *entry* assembly in order to load 
